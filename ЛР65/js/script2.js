...
function findValidN() {
  ...
  for (let n = 1; n <= 5; n++) {
    if (((newFrequency / n) - 0.5) <= frequency && frequency <= ((newFrequency / n) + 0.5)) {
      isValidN = true;
      validN = n;

      break; // Найдено подходящее значение, выходим из цикла
    }
  }
}
...
function toggleStringImage() {
  ...
  if (!isOn) {
    ...
    imagePath = `string-on-${validN}.png`;
  } else if (((newFrequency / validN) - 0.5) <= frequency) {
    imagePath = 'string-on-0.png';
  } else if (frequency <= ((newFrequency / validN) + 0.5)) {
    imagePath = 'string-on-0.png';
  } else {
    imagePath = 'string-on-0.png';
  }
  console.log("n:", validN);
